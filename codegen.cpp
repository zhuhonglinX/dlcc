...
string CodeGen::get_jmp_label() {
    return "L" + to_string(jmp_cnt++);
}
    ...
    switch (node->get_ast_type()) {
        ...
        case AstType::CALLFUNC : {
            ...
            if (call->fname_ == "printf") {
                vm_code.emplace_back(make_pair(Ins::PRINT, to_string(call->args_.size())));
                break;
            }
            // Ins: CALL <func id>
        ...
        case AstType::STR : {
            auto *ast = dynamic_cast<StrAst *>(node);
            // Ins: PUSHS <str>
            vm_code.emplace_back(make_pair(Ins::PUSHS, to_string(ast->data_offset_)));
            break;
        }
        case AstType::RET : {
            auto *ret = dynamic_cast<ReturnAst *>(node);
